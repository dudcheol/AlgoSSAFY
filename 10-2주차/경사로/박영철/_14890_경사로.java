			...
			for (int j = 1; j < N; j++) {
				...
				if (dif == 1) { // 차이가 1나면 경사로 놓기를 시도해본다
					...
					if (map[i][j - 1] > map[i][j]) { // 왼쪽이 더 높다면 현재칸부터 L-1칸 오른쪽이 같아야함
						// 이 칸에서부터 L만큼 같은 숫자가 나와야 경사로 놓을 수 있음
						for (int l = 0; l < L; l++) {
							int nj = j + l;
							if (nj >= N)
								continue loop; // 범위를 벗어남
							if (map[i][j] != map[i][nj] || visited[i][nj] != 0) // 높이가 다르거나, 이미 지어진 경사로가 있다면 경사로 지을 수 없음
								continue loop;
						}
						// 경사로 지을 수 있으면 visited에 표시하기
						for (int l = 0; l < L; l++) {
						...
						for (int l = 0; l < L; l++) {
							...
							if (nj < 0)
								continue loop;
							if (map[i][j - 1] != map[i][nj] || visited[i][nj] != 0)
								continue loop;
						}
						// 경사로 지을 수 있으면 visited에 표시하기
						for (int l = 0; l < L; l++) {
				...
				if (dif == 1) { // 차이가 1나면 경사로 놓기를 시도해본다
					...
					if (map[j - 1][i] > map[j][i]) { // 위쪽이 더 높다면 현재칸부터 L-1칸 아래쪽이 같아야함
						...
						for (int l = 0; l < L; l++) {
							...
							if (nj >= N)
								continue loop; // 범위를 벗어남
							if (map[j][i] != map[nj][i] || visited[nj][i] != 0) // 높이가 다르거나, 이미 지어진 경사로가 있다면 경사로 지을 수 없음
								continue loop;
						}
						// 경사로 지을 수 있으면 visited에 표시하기
						for (int l = 0; l < L; l++) {
						...
						for (int l = 0; l < L; l++) {
							...
							if (nj < 0)
								continue loop;
							if (map[j - 1][i] != map[nj][i] || visited[nj][i] != 0)
								continue loop;
						}
						// 경사로 지을 수 있으면 visited에 표시하기
						for (int l = 0; l < L; l++) {
